...
class Omok:
    ...
    def draw(self):
        ...
        if self.lastPos != None and self.frames%(FPS) < FPS*3//4:
            ...
            pygame.draw.circle(self.display, (255, 0, 0),
                (Margin+CellSize2+c*CellSize, Margin+CellSize2+r*CellSize), 5)
    ...
    def onPlace(self, r, c):
        # (r, c) 위치에 turn(검은돌 or 흰돌)을 놓습니다.
        self.board[r][c] = self.turn
        # 마지막에 놓은 돌의 위치 저장
        self.lastPos = (r, c)
        # 마지막 위치의 돌을 놓았을 때, 몇개가 같은 줄에 있는지 셉니다.
        drc = ((0, 1), (1, 1), (1, 0), (1, -1))
        for dr, dc in drc:
            count = 1
            for i in range(1, 6):
    ...
    def update(self):
        for e in pygame.event.get():
            ...
            if e.type == MOUSEBUTTONUP:
                ...
                # 영역밖이 클릭된 경우에는 처리하지 않습니다.
                if r < 0 or r >= 9 or c < 0 or c >= 9: continue
                # 이미 돌이 있는 경우에도 처리하지 않습니다.
...
game = Omok()
# quitFlag는 지연 종료를 위한 변수
quitFlag = -1
while quitFlag < FPS * 5:
